...
public class UserService {
    ...
    public UserService(UserRepository userRepository, BCryptPasswordEncoder bCryptPasswordEncoder, EmailAuthService emailAuthService) {
    ...
    private void checkEmailValid(String email) {
        ...
        if(email == null || !email.matches(EMAIL_FORMAT)){
            throw new UserException(UserExceptionType.INVALID_EMAIL_FORMAT);
        }
    }
    ...
    public CheckResponse changePassword(String email, String oldPassword, String newPassword) throws UserException {
        ...
        User user = userRepository.findByEmail(email)
                .orElseThrow(()-> new UserException(UserExceptionType.USER_NOT_EXIST));


        // 기존 비밀번호 확인
        if (!bCryptPasswordEncoder.matches(oldPassword, user.getPassword())) {
            throw new UserException(UserExceptionType.USER_WRONG_PASSWORD); // 기존 비밀번호가 틀린 경우
        }
    ...
    public CheckResponse deleteUser(String email, String password) throws UserException {
        ...
        if (!bCryptPasswordEncoder.matches(password, user.getPassword())) {
            throw new UserException(UserExceptionType.USER_WRONG_PASSWORD); // 비밀번호가 맞지 않으면 예외
        }
